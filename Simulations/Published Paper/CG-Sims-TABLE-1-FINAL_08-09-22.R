...
# Selection degree vector rho={0,0.25,0.5} (Note: will be over-written for CASE VII-VIII)
RHOvec <- c(0,0.25,0.5)

# Misspecification parameter vector gamma={0.25,0.5} (Note: will be over-written for other cases)
GAMMAvec <- c(0.25,0.5)
...
# Loop over Case 1-8 (see supplementary material, S13-S18):

for (case in 1:8) {
  
if (case<=6) {
  
for (sel in 1:length(RHOvec)) {
...
if (case<=4) {
  ...
  ######################################################
  # File saved as:                                     #
  # MC-Results-CQ-T1-DGP-CASE-XXX-Rho-YYY.csv          #
  #                                                    #
  # Format of the saved csv result file (CASE I-IV):   #
  # Rows:                                              #
  # - Row 1: chosen value of "rho"                     #
  # - Row 2: sample size n ("SS")                      #
  # - Row 3: results for c=3.5                         # 
  # - Row 4: results for c=4.0                         #
  # - Row 5: results for c=4.5                         #
  # Columns:                                           #
  # - Column 1 and 3: alpha=0.05                       #
  # - Column 2 and 4: alpha=0.10                       #
  ######################################################
  ...
  # MC-Results-CQ-T1-DGP-CASE-XXX-Rho-YYY.csv          #
  #                                                    #
  # Format of the saved csv result file (CASE V-VI):   #
  # Rows:                                              #
  # - Row 1: chosen value of "rho"                     #
  # - Row 2: sample size n ("SS")                      #
  # - Row 3: results                                   # 
...
if (case>6) {
  ...
  table=rbind(matrix(paste("gamma=",gamma,sep=""),1,4),matrix(paste("SS=",kronecker(SS,matrix(1,1,2)),sep=""),1,4),RejRate)
  

    
    # Save csv of results
    path=paste("MC-Results-CQ-T1-DGP-CASE-",case,"-Gamma-",(gamma*4),".csv",sep="")
    write.table(table,path, row.names=FALSE, col.names=FALSE, sep=",")
  ...
  ######################################################
  # File saved as:                                     #
  # MC-Results-CQ-T1-DGP-CASE-XXX-Gamma-YYY.csv        #
  #                                                    #
  # Format of the saved csv result file (CASE VII-VIII):#
  # Rows:                                              #
  # - Row 1: chosen value of "gamma"                   #
  # - Row 2: sample size n ("SS")                      #
  # - Row 3: results                                   # 
  # Columns:                                           #
  # - Column 1 and 3: alpha=0.05                       #
