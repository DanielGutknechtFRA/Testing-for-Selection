...
VBDNTest <- function(Y, IndX, IndZ, T1IndZ, qhat, taueval, B, stat) {
...
for (SE in 1:6) {
  ...
  for (ss in 1:dim(SS)[2]) {
    ...
    for (mc in 1:MC) {
      ...
      S <- ((0.75*(X-0.5)+(0.75*Z))>=E[,2])
        ...
        Y <- (X-0.5)^3 + (X-0.5)^2 + (X-0.5)  + 0.5*E[,1]
      ...
      if (CV==0) {  
        for (h in 1:length(BW)) {
          ...
          CQ_stat_1 = VBDNTest(Ytrim,IndX,IndZ,T1IndZ,qhat, tau_grid, B=Bsize,MD)
  ...
  # Save csv of results
  path=paste("MC-Results-CQ-T1-DGP-SE-",SE,"-CV-",CV,"-Rho-",(rho*4),".csv",sep="")
